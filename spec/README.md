...
## Разница между Spec, Plan, Tasks, Verify и Hotfix в spec-flow
...
- Отвечает на вопросы:
  ...
  - **Можно ли переносить фичу в архив spec-flow?**
...
- **Verify** = подтверждаем *что сделано*, фиксируем несоответствия и решение по архиву.
...
Доступны материалы: `spec.md`, `plan.md`, `tasks.md` этой фичи.
```
...
Нужно: актуализировать `spec.md` и `plan.md`, добавить задачи на восстановление мониторинга, отметить проверки в `tasks.md` и `verify-report.md`.
```
...
- **Верификацию проводите последовательно.** Проходите задачи в `tasks.md` по порядку, обновляйте чекбоксы и сохраняйте новые записи в `verify-report.md` с датой и действиями.
...
- [ ] Для verify-отчётов обновлены чекбоксы в `tasks.md` и добавлены свежие записи в `verify-report.md`.
- [ ] Для хотфиксов сохранена сводка `hotfix.md` со ссылками на обновлённые артефакты и результатами проверок.

Spec-flow служит единым источником истины: чем подробнее и яснее документы на каждом шаге, тем предсказуемее и быстрее проходит разработка, а verify-отчёты позволяют прозрачно зафиксировать готовность к архивированию.
