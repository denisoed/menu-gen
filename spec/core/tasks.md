...
4. В итоговом файле явно укажи, что для проверки списка задач нужно запустить `/spec/core/verify.md`, чтобы команда сделала это при работе с фичей.
5. Зафиксируй в разделе `## Definition of Done`, что запуск `/spec/core/verify.md` выполняется после закрытия всех задач, чтобы исполнители сделали это автоматически по завершении списка.
